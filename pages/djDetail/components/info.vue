	...
	<view style="margin-left: 10px;margin-right: 10px;">
		<view>
			...
			<view class="display" style="flex-wrap: wrap;">
				...
				<view style="width: 50%;">绝缘等级：{{info.insulation_class || ''}}</view>
				<view style="width: 50%;">防爆标志：{{info.explosion_proof || ''}}</view>
				<view style="width: 50%;">冷却方式：{{info.cooling_mode || ''}}</view>
				<view style="width: 50%;">工作制：{{info.work_system || ''}}</view>
				<view style="width: 50%;">安装方式：{{info.installation_mode || ''}}</view>
				<view style="width: 50%;">轴伸端轴承型号：{{info.bearing_type || ''}}</view>

				<view style="width: 50%;">非轴伸端轴承型号：{{info.no_bearing_type || ''}}</view>
				<view style="width: 50%;">润滑介质：{{info.lubricating_medium || ''}}</view>
				<view style="width: 50%;">注油时间间隔：{{info.oil_filling_interval || ''}}</view>
